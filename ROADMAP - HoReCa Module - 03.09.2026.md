...
(გადახდის) მომენტში ხდებოდა — მანამდე item უკვე `'sent'`-ზე მიდიოდა და
სამზარეულოც ხედავდა KDS-ზე, რაც ლოგიკურად არასწორია (სამზარეულო
იწყებდა კერძს, რომელიც არასდროს გაიყიდებოდა). გადამოწმდა: `orders.ts`-ს
`POST /orders/:id/items`-ს საერთოდ არ ჰქონდა არანაირი stock-შემოწმება
არცერთი პროდუქტის ტიპზე (ეს იყო წინარე, BOM-მდელი არქიტექტურა, არა
axali ხარვეზი) — stock-გეითი მხოლოდ checkout-ში (`sales.ts`) არსებობდა.

**გამოსწორდა:** `POST /orders/:id/items`-ში (`routes/orders.ts`) item-ის
დამატებისთანავე (`productCheck`-ის შემდეგ, `station`/`kitchen_status`-ის
მინიჭებამდე) `is_recipe_based === true` პროდუქტზე მოწმდება, საკმარისია
თუ არა თითოეული `recipe_items`-ის ingredient-ის `stock` — თუ არა, item
საერთოდ არ insert-დება (400 შეცდომა, არც `'pending'`, არც `'sent'`
სტატუსი არ ენიჭება). Regular (non-recipe) პროდუქტებზე ცვლილება არ
შეხებია — ეს scope მხოლოდ მომხმარებლის მიერ ნაპოვნ, recipe-based
შემთხვევას ეხება.

**ცნობილი შეზღუდვა (განზრახ, v1):** ეს მხოლოდ ვალიდაციაა, არა
"დაჯავშნა" — ingredient-ის `stock` აქ არ იკლებს (კვლავ checkout-ზე
იკლება, `sales.ts`-ში), ასე რომ თეორიულად ორმა ოფიციანტმა ერთდროულად
რომ დაამატოს ბოლო ულუფა, ორივემ შეიძლება გაიაროს ეს item-add-time
შემოწმება — საბოლოო, ატომური გარანტია კვლავ checkout-ის
`WHERE stock >= $1` update-ზეა დამოკიდებული (ეს არასდროს იცვლება).

`npx tsc --noEmit` (backend + frontend) — 0 შეცდომა ამ ცვლილების შემდეგაც.
